...
import ExecutionVisualizerModal from "../Tracer/ExecutionVisualizerModal.jsx";
import { runPythonTracer } from "../Tracer/pythonTracer.js";
...
export default function PurpleHazeEditor({
  onToggleChat,
  isChatOpen,
  onCodeChange,
}) {
  ...
  const [tracerSteps, setTracerSteps] = useState([]);
  const [tracerVarMeta, setTracerVarMeta] = useState({});
    ...
    const loadPyodide = async () => {
      try {
        if (window.loadPyodide) {
          setOutput((prev) => [
            ...prev,
            { type: "info", content: "Loading Python environment..." },
          ]);
          ...
          setOutput((prev) => [
            ...
            {
              type: "success",
              content: "Python environment ready! Interactive mode enabled.",
            },
          ]);
        }
      } catch (error) {
        setOutput((prev) => [
          ...prev,
          { type: "error", content: `Failed to load Python: ${error.message}` },
        ]);
      }
    };
    loadPyodide();
  ...
  const handleRun = async () => {
    if (!pyodideReady) {
      setOutput([
        {
          type: "error",
          content: "Python environment is still loading. Please wait...",
        },
      ]);
    ...
    try {
      ...
      pyodide.globals.set("stdout_callback", (text) =>
        outputCallback(text, "stdout"),
      );
      pyodide.globals.set("stderr_callback", (text) =>
        outputCallback(text, "stderr"),
      );
...
def transform_input_calls(code_str):
    ...
    try:
      ...
      const transformedCode = await pyodide.runPythonAsync(
        `transform_input_calls(${JSON.stringify(code)})`,
      );
...
`;
  ...
  const handleVisualize = async () => {
    ...
    try {
      ...
      alert(
        "Failed to generate flowchart. Make sure the backend server is running on port 4000.",
      );
  ...
  const handleTrace = async () => {
    ...
    // Pause any active run so the two Pyodide calls don't collide
    if (isRunning) {
      alert("Please wait for the current run to finish before tracing.");
      return;
    }
    setIsTracing(true);
    try {
      const { steps, varMeta } = await runPythonTracer(
        pyodideRef.current,
        code,
      );
      setTracerSteps(steps);
      setTracerVarMeta(varMeta);
      setShowTracer(true);
    } catch (err) {
      console.error("Step Tracer error:", err);
      alert("Failed to trace code: " + (err?.message ?? String(err)));
    } finally {
      setIsTracing(false);
    }
  };
  ...
  if (isMobile) {
    return (
      ...
      >
        <CodeXml size={64} color="#a855f7" style={{ marginBottom: "24px" }} />
        <h1
          style={{
            fontSize: "24px",
            fontWeight: 700,
        ...
        >
          Our code editor and visualization tools are optimized for larger
          screens to provide you with the best development experience.
        </p>
        <div
          style={{
            marginTop: "32px",
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          <button
            ...
            onMouseLeave={(e) => {
              if (serverStatus === "ready") {
                e.currentTarget.style.backgroundColor = showVisualizer
                  ? "rgba(168,85,247,0.25)"
                  : "rgba(168,85,247,0.15)";
                e.currentTarget.style.borderColor = "rgba(168,85,247,0.3)";
              }
          ...
          <button
            onClick={handleTrace}
            disabled={isTracing || !pyodideReady || isRunning}
            title={
              !pyodideReady
            ...
            style={{
              ...
              padding: "8px 16px",
              borderRadius: "8px",
              border: `1px solid ${showTracer ? "rgba(52,211,153,0.55)" : "rgba(52,211,153,0.3)"}`,
              backgroundColor: showTracer
                ? "rgba(52,211,153,0.25)"
                : isTracing
                  ? "rgba(52,211,153,0.2)"
                  : "rgba(52,211,153,0.1)",
              color: !pyodideReady || isRunning ? "#64748b" : "#6ee7b7",
              fontSize: "13px",
              fontWeight: 600,
              cursor:
                isTracing || !pyodideReady || isRunning
                  ? "not-allowed"
                  : "pointer",
              transition: "all 0.2s",
              fontFamily: "monospace",
            ...
            onMouseLeave={(e) => {
              if (pyodideReady && !isRunning) {
                e.currentTarget.style.backgroundColor = showTracer
                  ? "rgba(52,211,153,0.25)"
                  : "rgba(52,211,153,0.1)";
                e.currentTarget.style.borderColor = showTracer
          ...
          >
            ...
            {isTracing ? "Tracing…" : !pyodideReady ? "Loading…" : "π-tracer"}
          </button>
          ...
          <button
            ...
            style={{
              ...
              borderRadius: "8px",
              border: "1px solid rgba(168,85,247,0.3)",
              backgroundColor: outputPanelOpen
                ? "rgba(168,85,247,0.25)"
                : "rgba(168,85,247,0.15)",
              color: "#e9d5ff",
              fontSize: "13px",
            ...
            onMouseLeave={(e) => {
              e.currentTarget.style.backgroundColor = outputPanelOpen
                ? "rgba(168,85,247,0.25)"
                : "rgba(168,85,247,0.15)";
              e.currentTarget.style.borderColor = "rgba(168,85,247,0.3)";
            }}
          ...
          <button
            ...
            onMouseLeave={(e) => {
              if (serverStatus === "ready") {
                e.currentTarget.style.backgroundColor = isChatOpen
                  ? "rgba(168,85,247,0.25)"
                  : "rgba(168,85,247,0.15)";
                e.currentTarget.style.borderColor = "rgba(168,85,247,0.3)";
              }
      ...
      <div
        style={{
          ...
          height: outputPanelOpen
            ? `calc(100% - ${outputPanelHeight}px - 26px)`
            : "auto",
        }}
      >
        <Editor
      ...
      {/* ── Step-by-Step Execution Tracer Modal ─────────────────────────── */}
      <ExecutionVisualizerModal
        isOpen={showTracer}
        onClose={() => setShowTracer(false)}
        steps={tracerSteps}
        varMeta={tracerVarMeta}
        code={code}
      />
          ...
          >
            {output.length === 0 ? (
              ...
              output.map((item, index) => (
                ...
                >
                  {item.type === "input" && (
                    <span style={{ color: "#8b5cf6", marginRight: "8px" }}>
                      {">"}
                    </span>
                  )}
                  {item.content}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
          <div
            style={{
              ...
              alignItems: "center",
              gap: "6px",
              color: pyodideReady ? "#22c55e" : "#fbbf24",
            }}
          >
            <div
              style={{
